...
- [読書の手帖（Android版）](#読書の手帖android版)
  - [1. 説明](#1-説明)
    - [1-1. 作成した理由](#1-1-作成した理由)
    - [1-2. 書誌・書影](#1-2-書誌書影)
    - [1-3. プライバシーポリシー](#1-3-プライバシーポリシー)
    - [1-4. 本アプリのバージョン](#1-4-本アプリのバージョン)
  - [2. 使い方](#2-使い方)
...
なお、**読書の手帖**は、書影画像（資料の表紙の画像）を取得しません。
...
- 他のアプリでISBNを共有した場合に、![読み取る](../../common/images/content_paste_32dp_000000_FILL0_wght400_GRAD0_opsz40.png)をタップすると**読書の手帖**はコピーしたISBNを使って検索を行い結果を表示します。
...
- [国立国会図書館サーチ](https://ndlsearch.ndl.go.jp/)で本を検索して、見つかった本のページにある「引用文（参考文献注）を生成」をタップして、「出典の記載例」を全て選択して共有した場合に、![読み取る](../../common/images/content_paste_32dp_000000_FILL0_wght400_GRAD0_opsz40.png)をタップすると**読書の手帖**は、引用文をもとに詳細ページに表示します。
...
タイトルおよび著者をタップするとPopupを表示します。タイトル、著者を指定して![apply](../../common/images/check_32dp_1F1F1F_FILL0_wght400_GRAD0_opsz40.png)をタップしてください。変更を破棄して元の表示に戻るには、![cancel](../../common/images/cancel_32dp_1F1F1F_FILL0_wght400_GRAD0_opsz40.png)をタップします。
...
なお、登録してある本の記録をタップして[詳細画面](#3-1-2-本の記録を記入編集削除する)を表示した場合には、![Delete](../../common/images/delete_32dp_1F1F1F_FILL0_wght400_GRAD0_opsz40.png)をタップすると記録を**削除**します。
...
状態では**チェックあり**の状態だけを一覧に表示します。タップしてチェックありなしを切り替え、![filter](../../common/images/filter_alt_32dp_1F1F1F_FILL0_wght400_GRAD0_opsz40.png)をタップすると、一覧の表示を更新します。

![BottomSheet](./images/resized/01_Records_N230_BooksPageFilter.png)

[読了年]に年を指定して、![filter](../../common/images/filter_alt_32dp_1F1F1F_FILL0_wght400_GRAD0_opsz40.png)をタップすると、一覧に指定した年に読み終わった本の記録だけ表示します。一番面白かった本を検討する際に、一年間に読んだ本だけ表示できるように作りました。

[賞]は[スイッチ]をオンにして、![filter](../../common/images/filter_alt_32dp_1F1F1F_FILL0_wght400_GRAD0_opsz40.png)をタップすると、一覧に賞有りだけ表示します。
...
|読了日|読み終わった（本の記録に**読み終わった**を設定した）順に並び替えます|
...
右上[メニュー]の[初期設定に戻す]か![Restore](../../common/images/reset_settings_32dp_1F1F1F_FILL0_wght400_GRAD0_opsz40.png)をタップするとフィルターと並び替えを初期設定に戻し、一覧を更新します。
...
- 国立国会図書館サーチおよび青空文庫・国立国会図書館サーチの一回の検索で取得できる結果は最大500個です。

  `検索負荷回避のための制約により、501件目以降を指定した場合でも該当のデータを取得することはできない。`
...
- **読書の手帖**でインポートする
  ...
  3. Dropboxを起動する（読書管理ビブリアでのDropboxと同じアカウントで）
  4. Dropboxで「Dropbox/アプリ/Biblia/books.csv」を選択して「デバイスに保存」でローカルフォルダー（ダウンロード等）に出力する
...
- **読書の手帖**でエクスポートして、読書管理ビブリアで復元する
  ...
  3. Dropboxを起動する（読書管理ビブリアでのDropboxと同じアカウントで）
  4. Dropboxで「Dropbox/アプリ/Biblia」に上のbooks.csvをアップロードする、この時失敗に備えて既存ファイルはリネームしておく
