...
バーコード読み取りで利用するML Kit APIは、バグ修正、モデルの更新、ハードウェアアクセラレータの互換性情報などを取得するために、随時Googleのサーバーに接続する場合があります。また、ML Kit APIは、アプリ内でのAPIのパフォーマンスや利用状況に関するメトリクスデータをGoogleに送信します。Googleは、このメトリクスデータを使用して、APIのパフォーマンス測定、デバッグ、保守、改善を行うほか、Googleの[プライバシーポリシー](https://policies.google.com/privacy)に詳述されているとおり、不正使用や悪用を検知します。
